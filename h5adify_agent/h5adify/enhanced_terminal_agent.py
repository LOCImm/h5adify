"""
Enhanced Terminal Agent for h5adify v5.0.0
Complete implementation with model selection and improved help
"""
...
class H5ADEnhancedTerminalAgent:
    """Enhanced terminal agent with all v5.0.0 features."""
    ...
    def __init__(self):
        ...
        self.command_handlers = {
            ...
            CommandType.MODEL_SELECT: self.handle_model_select,
            CommandType.MODELS: self.handle_models,
        }
    ...
    def display_banner(self):
        ...
        print("🤖 Enhanced h5adify Terminal Agent v5.0.0")
        print("=" * 60)
    ...
    def handle_models(self, args: List[str]) -> bool:
        """Handle models command to list available AI models."""
        if not self.ollama.available:
            print("❌ AI models not available. Please install and start Ollama.")
            return False
        ...
        for i, model in enumerate(available_models, 1):
            current_marker = " ← Current" if model == self.ollama.model else ""
            print(f"{i}. {model}{current_marker}")
        
        print(f"\n💡 Use 'model_select <model_name>' to switch models")
        return True
    
    def handle_model_select(self, args: List[str]) -> bool:
        """Handle model_select command."""
        if not args:
            print("❌ Model name required. Use 'model_select <model_name>'")
        ...
        if self.ollama.set_model(model_name):
            print(f"✅ AI model switched to: {model_name}")
            return True
        else:
    ...
    def handle_ai_annotate(self, args: List[str]) -> bool:
        """Handle AI annotate command with enhanced help."""
        if not args:
            print("🤖 AI-Powered Paper Annotation")
            print("=" * 50)
            print("Extract structured metadata from research papers using AI.")
            print()
            print("Supported formats:")
            print("  • DOI: 10.1038/nature12373")
            print("  • PubMed ID: 12345678") 
            print("  • Full URL: https://doi.org/10.1038/nature12373")
            print()
            print("Extracted information:")
            print("  • Species/organism studied")
            print("  • Sequencing technology used")
            print("  • Sample/cell count")
            print("  • Tissue or cell type")
            print("  • Disease state (if applicable)")
            print("  • Experimental design")
            print("  • Key biological findings")
            print("  • Dataset type")
            print()
            print("Usage: ai_annotate <doi|url>")
            return True
        ...
        if not self.ollama.available:
            ...
            print("Current model:", self.ollama.model)
            return False
    ...
    def handle_help(self, args: List[str]) -> bool:
        """Handle help command with enhanced documentation."""
        print("🤖 Enhanced h5adify Terminal Agent v5.0.0 - Help")
        print("=" * 70)
        print()
        
        print("🎯 OVERVIEW:")
        print("h5adify is a comprehensive single-cell data processing toolkit with")
        print("GUI and terminal interfaces for searching genomic databases, managing")
        print("local files, and AI-powered annotation.")
        print()
        ...
        print("  ai_annotate               Show AI annotation help")
        ...
        print("  model_select <name>       Switch to different AI model")
        print()
        ...
        print("  • GEO: NCBI Gene Expression Omnibus")
        print("  • UCSC: UCSC Cell Browser")
        print("  • Zenodo: Open access research repository")
        print("  • EMA: EBI Expression Atlas")
        print("  • CellxGene: Chan Zuckerberg CellxGene")
        print("  • SCP: Broad Institute Single Cell Portal")
        print()
        
        print(f"🤖 Current AI Model: {self.ollama.model}")
        print(f"Available sources: {', '.join(self.get_available_sources())}")
    ...
    # Include all other handler methods from the previous implementation
    # (open_link, verbose, json, list_local, query_local, etc.)
...
Provide concise, accurate information about single-cell RNA sequencing, transcriptomics, and related technologies."""
    ...
    def run(self):
        """Run the enhanced terminal agent."""
        self.display_banner()
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Enhanced h5adify Terminal Agent v5.0.0")
    ...
    if args.gui:
        ...
        agent = H5ADEnhancedTerminalAgent()
